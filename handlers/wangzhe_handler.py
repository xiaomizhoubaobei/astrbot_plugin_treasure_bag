        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(api_url) as resp:
                if resp.status == 200:
                    ...
                    logger.info(
                        f"成功查询英雄 {hero_name} 的资料: {data_text[:100]}..."
                    )
                    if data_text:
                        ...
                        yield event.plain_result(
                            f"未能查询到英雄 {hero_name} 的资料，可能是英雄名称错误或API暂无数据。"
                        )
                else:
                    logger.error(
                        f"请求王者荣耀API失败，英雄：{hero_name}，状态码: {resp.status}"
                    )
                    yield event.plain_result(
                        f"抱歉，查询英雄 {hero_name} 资料失败了 (状态码: {resp.status})。"
                    )
    except Exception as e:
        logger.error(f"请求王者荣耀API时发生错误，英雄：{hero_name}: {e}")
        yield event.plain_result(
            f"抱歉，查询英雄 {hero_name} 资料时发生了意料之外的错误。"
        )
