...
def generate_face_analysis_report(face_info):
    ...
    quality_parts.append(f"  清晰度：{blur_status} (值：{blur:.2f})")
    quality_parts.append(f"  光照：{light_status} (值：{illumination})")
    quality_parts.append(f"  完整度：{'完整' if completeness == 1 else '不完整'}")
    ...
    if liveness_score != -1:
        ...
        if "faceliveness" in liveness_details:
            quality_parts.append(f"  活体检测详细得分：{liveness_details['faceliveness']:.2f}")
        if "faceliveness_threshold" in liveness_details:
            quality_parts.append(f"  活体检测阈值：{liveness_details['faceliveness_threshold']:.2f}")
    
    # 2. 主报告部分
    result_parts.append("✨ 颜值分析报告 ✨")
    result_parts.append("--------------------")
