...
class BaiduAuthManager:
    ...
    def init_credentials(self, config: AstrBotConfig):
        ...
        if not self.api_key or not self.secret_key:
            ...
            logger.info(f"百度API凭证已从插件配置加载。API Key: {self.api_key[:4]}...{self.api_key[-4:]}, Secret Key: {self.secret_key[:4]}...{self.secret_key[-4:]}")
    ...
    async def get_access_token(self):
        ...
        async with self._lock:  # 使用锁确保在并发场景下只有一个请求获取新token
            ...
            logger.info(f"正在请求百度access_token, 参数: grant_type=client_credentials, client_id={self.api_key[:4]}..., client_secret={self.secret_key[:4]}...")
            try:
                    ...
                    async with session.post(token_url, params=params) as resp:
                        if resp.status == 200:
                            ...
                            if "access_token" in token_data:
                                ...
                                self.token_expiry_time = current_time + token_data.get("expires_in", 2592000)
                                logger.info(f"成功获取百度 access_token，有效期至: {datetime.fromtimestamp(self.token_expiry_time)}")
                                return self.access_token
                            else:
